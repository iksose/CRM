angular.module('uiRouterSample')
    .controller('searchController', function($scope, $rootScope, $state, $alert, searchFactory, $timeout, $location) {
        ...
        $scope.selectedProspectType = $scope.prospectType.map(type => type.value)
        $scope.customerType = [{
            value: 'P',
            label: 'ProfitGuard'
        }, {
        ...
        $scope.selectedCustomerType = $scope.customerType.map(type => type.value)
        ...
        //             value
        //         }
        //         of $scope.customerType) value
        // ]

        $scope.BDMs = ['BDM01', 'BDM02', 'BDM03', 'BDM04']
        $scope.selectedBDM = [
            for (x of $scope.BDMs) x
        ]
        ...
        if (Object.keys(stateParams).length) {
            ...
            searchFactory.search(stateParams).then((res) => {
                ...
                if (res.data.length > 0) {
                    ...
                    $scope.searchResults = res.data.map(searchResult => new Prospect(searchResult))
